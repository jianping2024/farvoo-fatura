...
| 窗口单例 | **同进程：** 第二次「开票」→ `RequestOpen` **聚焦**已有 HWND（无响应则丢弃重建）。**跨进程：** Agent 桌面快捷方式 `fiscal` → IPC `RequestOpenFiscal`（不启第二 Agent）；Client → Mutex + `FocusExistingByTitle`（不启第二 Client）。重复启动 Agent → **静默退出**（无弹窗） |
