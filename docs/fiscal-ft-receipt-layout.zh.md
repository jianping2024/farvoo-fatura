...
| 9 | **店名**仅 `LegalName`：加粗 + 1×2（`GS ! 0x01`），印完恢复 1×1；店名下 **半行进纸**（`ESC d 15`，15 点）；地址 / NIF / BusinessName 仍 1×1 | 王氏抬头观感；禁止把整段抬头都放大 |
...
| 店名后间距 | 仅 `escFeedDots(receiptTopGapDots)` |
...
7. 店名后字节序含 `ESC d` + `receiptTopGapDots`（15），非整行 `LF`
...
| 2026-09-01 | P0 #11：店名后半行进纸（15 点）；QR 后切前进纸 **85 点**（原 170 点之半）；`cutFeedDots=55` |