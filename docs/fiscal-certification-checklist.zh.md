...
> **状态：定稿**（M6 D6.2–D6.4 + P0 认证阶段关闭；**FR 产品化**后范围含 FR）  
> **权威范围：** 产品认证单据 **FT + FS + FR + NC + ND**（账单同步签发仍仅 FT+FS；FR 为手工/API 销售票）  
...
> **设计：** [`fiscal-m6-backup-swap.zh.md`](fiscal-m6-backup-swap.zh.md)（D6.3/D6.4）；FR 产品定法见 [`fiscal-m6-fs-fr-nd.zh.md`](fiscal-m6-fs-fr-nd.zh.md)
...
| C2.2 | 产品类型限制 | API/手工可签 **FR**；UI 销售下拉 FT、FS、FR；账单同步仍仅 FT、FS | 自动 + 手测 | d62 签 FR；**手测** Admin 下拉 |
...
| # | 检查项 | 通过标准 | 分类 | 验证方式 |
|---|--------|----------|------|----------|
| C5.1 | 月导含 FT/FS/FR/NC/ND | XML `InvoiceType` 齐全 + VALID | 自动 | d62 export + download |
...
1. **C2.2-UI** — Admin 手工开票类型可见 FT / FS / FR（账单分单仍仅 FT / FS） — **通过**（2026-09-02 时为 FT/FS；FR 产品化后需复核下拉）
