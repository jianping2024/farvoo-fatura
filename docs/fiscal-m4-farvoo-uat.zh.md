...
> **状态：已通过**（联调执行记录）  
> **权威：否** — 手测清单；产品路径见 restaurant-ordering [`farvoo-fiscal-bill-sync-api.zh.md`](../../restaurant-ordering/docs/technical/farvoo-fiscal-bill-sync-api.zh.md)；Agent 见 [`fiscal-bill-draft-workbench.zh.md`](fiscal-bill-draft-workbench.zh.md)  
> **环境：** Farvoo **cloud** · 门店 **白云饭店**  
> **Agent：** 托盘主进程（非 `fiscal-local` seed）；Admin **0.4.20** 餐馆模式

## 1. 范围

| 用例 | 说明 |
|------|------|
| 整桌 | Farvoo **同步关台** → `scope_type=whole_table` → 收银账单 → Admin 签发一张 FT |
| 按人 | 同步载荷 `scope_type=split` + `splits[]` → 选人 → 各签一张 FT；互斥规则见 workbench §5.2 |

**不在范围：** Farvoo 桌台「打印发票」直调 Agent Local API（餐馆产品不做）。
...
| 项 | 说明 |
|----|------|
| Cloud by_item | 若 cloud 只发 whole_table，按人用例依赖 cloud 侧 split 载荷 |
| 本机 by-item 分单工作台 | 另轨（M2.6 UX）；与 cloud split 载荷不同 |

## 6. 执行记录

| 日期 | 执行人 | 整桌 | 按人 | 备注 |
|------|--------|------|------|------|
| 2026-08-29 | Will | 通过 | 通过 | Agent ~0.4.20 |
...
| 2026-08-29 | 联调通过 |
| 2026-08-30 | 去掉 sync_outbox/§13 缺口；明确不做桌台直连接口 |
