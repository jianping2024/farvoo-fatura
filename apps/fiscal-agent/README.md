...
| `FISCAL_ALLOW_LAN` / `FISCAL_BIND` | No (CLI only) | **Agent product path ignores these.** Use Admin → Invoice PCs (`fiscal_allow_lan` in `config.json`). `fiscal-local` may still set them for UAT. |
