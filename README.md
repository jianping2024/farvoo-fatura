...
**Farvoo Local Agent**：门店本机 **厨打打印** Agent；**同进程可选**嵌入合规开票（Fiscal）。

业务热敏（厨打/预结等）与税务小票共用同一 Agent、同一套打印机绑定与 ESC/POS 能力。**未开通打票的店**只需配对 + 映射打印机，可忽略开票入口。

## 定位（定稿）

| 项 | 定法 |
|----|------|
| 本仓职责 | 门店 **打印模块**（业务打印；**打票为可选产品**） |
| 打票（Fiscal） | **可选**：Ops/Dashboard 开通 + Admin 激活后才合规开票；工程上同进程嵌入，**不等于**每店必配 |
| 发票出纸 | 开通后：Fiscal Core 签发 → **同一进程**本地打印队列出纸 |
